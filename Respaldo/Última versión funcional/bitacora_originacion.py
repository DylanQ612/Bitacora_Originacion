...
# ========== INTERFAZ STREAMLIT ==========
st.title("Departamento de Originación - Bitácora de actividades")
...
if pagina == "Bitácora de Actividades":
    ...
    with st.form("registro_form", clear_on_submit=True):  
        ...
        with col3:
            tipo_cliente = st.selectbox("Tipo de Cliente", ["NUEVO", "NUEVO INACTIVO"])
        ...
        with col5:
            ejecutivo = st.selectbox("Analista", [
                "Arleny Yaneth Carrillo Carrillo",
                "Angel Gabriel Gamez Garcia",
                "Guillermo Alexis Ayon Carrasco",
                "Braulio Lopez Chavira",
                "Isela Elizabeth Ortiz Siari",
                "Edgar Adolfo Quiñonez Corral",
                "Gilberto Bernal Corral",
                "Gladyss Vanessa Cortes Madrid",
                "Jazmin Guadalupe Bermudez Meza",
                "Kassandra Escarlett Torres Figueroa",
                "Karime Stefania Villalobos Félix",
                "Lourdes Guadalupe Felix Torres",
                "Mariana Adilene Bastidas Zazueta",
                "Maria Guadalupe Beltran Ruiz",
                "Miguel Ernesto García Alcalá",
                "Maria Fernanda Zazueta Aguilar",
                "Naidelin Bejarano Gamez"
            ])
    ...
    if submit_button:
        ...
        if ejecutivo not in ejecutivos_validos:
            st.error("⚠️ El analista seleccionado no es válido. Por favor selecciona uno de la lista.")
        else:
            conn = get_connection()
            if conn:
                try:
                    # Primero, obtener el LC_Disponible de la tabla LC_Disponible
                    lc_disponible = None
                    try:
                        query_lc = text("SELECT LC_Disponible FROM LC_Disponible WHERE ID_CLIENTE = :cliente")
                        result_lc = conn.execute(query_lc, {"cliente": cliente})
                        row_lc = result_lc.fetchone()
                        if row_lc:
                            lc_disponible = row_lc[0]
                    except Exception as e:
                        st.warning(f"⚠️ No se pudo obtener LC_Disponible: {e}")

                    # Obtener el siguiente valor de Registro
                    next_registro = None
                    try:
                        query_max_registro = text("SELECT ISNULL(MAX(Registro), 0) + 1 FROM Bitacora_Originacion")
                        result_max = conn.execute(query_max_registro)
                        row_max = result_max.fetchone()
                        if row_max:
                            next_registro = row_max[0]
                    except Exception as e:
                        st.error(f"⚠️ No se pudo obtener el siguiente Registro: {e}")
                        raise

                    # Luego, insertar el registro con el LC_DISPONIBLE obtenido y el Registro
                    query = text("""
                        INSERT INTO Bitacora_Originacion (
                            Registro, FECHA, TICKET, SUC, CLIENTE, VENTA, MOTO,
                            TIPO_DE_CLIENTE, NOTAS, LC_ACTUAL, LC_FINAL,
                            ENGANCHE_REQUERIDO, ENGANCHE_RECIBIDO, OBSERVACION, ESPECIAL,
                            ARTICULO, EJECUTIVO, CEL_CTE, CONSULTA_BURO, Actualizacion, FACTURO, innecesario, LC_DISPONIBLE
                        )
                        VALUES (:registro, :fecha, :ticket, :sucursal, :cliente, :venta, :moto,
                                :tipo_cliente, :notas, :lc_actual, :lc_final,
                                :enganche_requerido, :enganche_recibido, :observacion, :especial,
                                :articulo, :ejecutivo, :cel_cte, :consulta_buro, :actualizacion, :facturo, :innecesario, :lc_disponible)
                    """)
                    # Obtener la hora actual en la zona horaria de Culiacán
                    hora_actual = datetime.now(culiacan_tz).time()

                    # Combinar la fecha seleccionada con la hora actual
                    fecha_hora = culiacan_tz.localize(datetime.combine(fecha, hora_actual))

                    # Debug: Mostrar la fecha/hora que se va a insertar
                    st.info(f"🔍 DEBUG: Insertando fecha/hora: {fecha_hora} (Tipo: {type(fecha_hora)})")

                    conn.execute(query, {
                        "registro": next_registro,
                        "fecha": fecha_hora,
                        "ticket": ticket,
                        "sucursal": sucursal,
                        "cliente": cliente,
    ...
    with col3:
        filtro_ejecutivo = st.selectbox("Filtrar por Analista", ["Todos"] + [
            "Arleny Yaneth Carrillo Carrillo",
            "Angel Gabriel Gamez Garcia",
            "Guillermo Alexis Ayon Carrasco",
            "Braulio Lopez Chavira",
    ...
    with col_eliminar:
        ...
        if not df_records.empty:
            ...
            if st.button("Eliminar Registro"):
                ...
                if conn:
                    try:
                        delete_query = text("DELETE FROM Bitacora_Originacion WHERE Registro = :registro")
    ...
    if not bitacora.empty:
        ...
        with col2:
            st.subheader("Distribución por analista (clientes sin compra)")
            ...
            # 4. Total de registros capturados por analista (del universo total en el mes)
            total_registrados_por_ejecutivo = bitacora_mes.groupby("EJECUTIVO")["CLIENTE"].nunique().reset_index().rename(columns={"CLIENTE": "Registros", "EJECUTIVO": "ANALISTA"})

            # 5. Registros con VENTA = 'NO AUTORIZADA'
            no_autorizada_df = bitacora_mes[bitacora_mes["VENTA"] == "NO AUTORIZADA"]
            no_autorizada_por_ejecutivo = no_autorizada_df.groupby("EJECUTIVO")["CLIENTE"].nunique().reset_index().rename(columns={"CLIENTE": "No aut.", "EJECUTIVO": "ANALISTA"})

            # 6. Registros sin compra
            sin_compra_por_ejecutivo = sin_compra_df.groupby("EJECUTIVO")["CLIENTE"].nunique().reset_index().rename(columns={"CLIENTE": "Sin compra", "EJECUTIVO": "ANALISTA"})

            # 7. Unir todo en un resumen
            resumen_ejecutivo = total_registrados_por_ejecutivo.merge(
                no_autorizada_por_ejecutivo, on="ANALISTA", how="left"
            ).merge(
                sin_compra_por_ejecutivo, on="ANALISTA", how="left"
            )
            ...
            resumen_ejecutivo = resumen_ejecutivo[["ANALISTA", "Registros", "No aut.", "Sin compra", "% Sin compra"]]
    ...
    with col2:
        filtro_ejecutivo = st.selectbox("Analista", ["Todos"] + sorted(sin_compra_df["EJECUTIVO"].dropna().unique()))
